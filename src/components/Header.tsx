...
export const Header: React.FC<HeaderProps> = ({ onConsultClick }) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 transition-all duration-300">
      ...
      <div className="bg-[#070c1b] text-slate-300 text-xs border-b border-slate-800/80 px-4 py-1.5 hidden md:block">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-6">
            ...
            <span className="text-slate-400">
              Office: <strong className="text-slate-200 font-normal">Sector 43-B, Chandigarh</strong>
            </span>
            <span className="text-slate-500">|</span>
            <span className="text-[#c5a059] font-medium">Service Areas: Chandigarh • Mohali • Panchkula • Yamunanagar</span>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-slate-400">24 × 7 Legal Assistance</span>
          </div>
        </div>
      </div>
          ...
          <div className="hidden lg:flex items-center space-x-1 xl:space-x-2">
            {navLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className="px-3 py-2 text-sm font-sans font-medium text-slate-200 hover:text-[#d4af37] transition-colors rounded-md hover:bg-white/5"
              >
                {link.label}
              </a>
            ))}
          ...
          <div className="lg:hidden bg-[#070c1b] border-b border-slate-800 px-4 pt-3 pb-6 space-y-3 animate-in fade-in slide-in-from-top-4 duration-200">
            <div className="grid grid-cols-2 gap-2 pt-2 border-b border-slate-800/80 pb-3">
              {navLinks.map((link) => (
                <a
                  ...
                  className="px-3 py-2 text-sm text-slate-300 hover:text-[#d4af37] hover:bg-slate-900 rounded-md font-medium"
                >
                  {link.label}
                </a>
              ))}
            ...
            <div className="pt-2 flex flex-col gap-2.5">
              <div className="text-xs text-slate-400 text-center">
                Office: Sector 43-B, Chandigarh • High Court Litigation
              </div>
              <div className="grid grid-cols-2 gap-2">
                <a
                  href={ADVOCATE_CONFIG.telUrl}
                  className="flex items-center justify-center gap-2 py-2.5 px-3 rounded-lg bg-slate-900 border border-slate-700 text-slate-200 text-xs font-semibold"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 py-2.5 px-3 rounded-lg bg-[#25D366]/15 border border-[#25D366]/40 text-[#25D366] text-xs font-semibold"
                >
                  <MessageSquare className="w-3.5 h-3.5" />
                  WhatsApp
                </a>
